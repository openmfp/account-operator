...
func RunController(_ *cobra.Command, _ []string) { // coverage-ignore
	...
	var provider *apiexport.Provider
	...
	if len(operatorCfg.Kcp.ApiExportEndpointSliceName) > 0 {
		...
		provider, err = apiexport.New(mgrConfig, apiexport.Options{})
		if err != nil {
			log.Fatal().Err(err).Msg("unable to construct cluster provider")
		}
	}

	mgr, err = mcmanager.New(mgrConfig, provider, opts)
	if err != nil {
