...
func NewAuthorizationModelReconciler(mgr ctrl.Manager, log *logger.Logger) *AuthorizationModelReconciler {

	subs := []lifecycle.Subroutine{}

	return &AuthorizationModelReconciler{
		lifecycle: lifecycle.NewLifecycleManager(log, operatorName, "AuthorizationModelReconciler", mgr.GetClient(), subs).WithConditionManagement(),
	}
}
